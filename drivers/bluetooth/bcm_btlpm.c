...
#if defined(CONFIG_BT_WAKE_AXP_GPIO)
...
#if defined(CONFIG_BT_WAKE_AXP_GPIO)
static void hostwake_work(struct work_struct *work);
DECLARE_DELAYED_WORK(hostwake_workqueue, hostwake_work);
#else
...
DECLARE_DELAYED_WORK(tx_workqueue, tx_work);
#endif
 ...
 */
#if defined(CONFIG_BT_WAKE_AXP_GPIO)
static unsigned long axp_gpio_get_value(unsigned axp_gpio)
{
	unsigned long val;

	val = gpio_get_value(axp_gpio);
	val = (val==2) ? 1 : 0;
	return val;
}
#endif
...
static inline int bluesleep_can_sleep(void)
{
	...
	return (gpio_get_value(bsi->ext_wake) != bsi->ext_wake_assert) &&
		...
		(bsi->uport != NULL);
#else
	return (axp_gpio_get_value(bsi->host_wake) != bsi->host_wake_assert) &&
...
static void hostwake_work(struct work_struct *work)
{
	...
	if (axp_gpio_get_value(bsi->host_wake) == bsi->host_wake_assert){
		bluesleep_rx_busy();
	}else{
		bluesleep_rx_idle();
	}
...
static int bluesleep_start(void)
{
	...
	BT_ERR("prepare request irq!, host_wake=%d", bsi->host_wake);
#else
	retval = axp_gpio_irq_request(0, bsi->host_wake,
		bluesleep_hostwake_isr, NULL);
	if (retval) {
		BT_ERR("Couldn't request axp BT_HOST_WAKE IRQ");
		goto fail;
	}
	BT_ERR("line=%d====GPIO1 VALUE=[%d]===", __LINE__, axp_gpio_get_value(bsi->host_wake));
	retval = axp_gpio_irq_set_type(0, bsi->host_wake,
		AXP_GPIO_IRQF_TRIGGER_RISING | AXP_GPIO_IRQF_TRIGGER_FALLING);
	if (retval) {
		BT_ERR("Couldn't set axp BT_HOST_WAKE IRQ type");
		goto fail;
	}

	BT_ERR("line=%d====GPIO1 VALUE=[%d]===", __LINE__, axp_gpio_get_value(bsi->host_wake));
	retval = axp_gpio_irq_enable(0, bsi->host_wake);
	if (retval < 0) {
		BT_ERR("Couldn't enable BT_HOST_WAKE as axp wakeup interrupt");
...
static void bluesleep_stop(void)
{
	...
	free_irq(bsi->host_wake_irq, NULL);
#else
	axp_gpio_irq_disable(0, bsi->host_wake);
	axp_gpio_irq_free(0, bsi->host_wake);
#endif
	wake_lock_timeout(&bsi->wake_lock, HZ / 2);
}
...
static int __init bluesleep_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		BT_ERR("can't request input direction bt_hostwake gpio %d\n",
		bsi->host_wake);
		return ret;
	}
#else
	ret = pinctrl_request_gpio(bsi->host_wake);
	if (ret < 0) {
		BT_ERR("can't request bt_hostwake axp gpio %d\n",
		bsi->host_wake);
		return ret;
	}
	ret = pinctrl_gpio_direction_input(bsi->host_wake);
	if (ret < 0) {
		BT_ERR("can't set bt_hostwake axp gpio %d input\n",
		bsi->host_wake);
		return ret;
	}
#endif
